...
OnePlayerGridPanel::OnePlayerGridPanel(wxNotebook *notebook, std::size_t width, std::size_t height, OnePBricksTextures& textures, BrickData& brickData):
	wxPanel{ notebook, wxID_ANY, wxPoint{-1, -1}, wxSize{EDITOR_GRID_WIDTH, EDITOR_GRID_HEIGHT} }, 
	darkGray{ wxT("#4f5049") },
	data{width, height},
	tex{textures},
...
void OnePlayerGridPanel::changeGridData(wxMouseEvent& event)
{
	...
	if( events.mouseState.LeftIsDown() && square.x < EDITOR_GRID_SQUARES_WIDTH && square.y < EDITOR_GRID_SQUARES_HEIGHT )
	{
...
void OnePlayerGridPanel::deleteGridData(wxMouseEvent& event)
{
	...
	if( events.mouseState.MiddleIsDown() && square.x < EDITOR_GRID_SQUARES_WIDTH && square.y < EDITOR_GRID_SQUARES_HEIGHT )
	{
...
{
	if( BRICKS::INDEX::RICH_BRICK == selectedBrick.index && squareCoords.x + 1 < EDITOR_GRID_SQUARES_WIDTH )
	{
