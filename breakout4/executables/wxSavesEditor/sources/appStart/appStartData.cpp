...
AppStartData::AppStartData():
	prefPath{ COMPANY_NAME, GAME_NAME, SQR_SIZE },
...
void AppStartData::loadTexts()
{
	if( logs )
	{
		...
		bagBonusesTexts.initializeTexts(logs.error, savesEditorDataPathStr + "savesEditorBagBonuses.txt", onePlGame::BONUS_MAX );
		//campaignLocaleTexts.initializeTexts(logs.error, getCampaignLocaleTextsFile("english", file::ONE_PLAY_SAVE_AND_LOAD), SAVE_NAME_FOCUSING_GAME_MAX);
	}
	else{
