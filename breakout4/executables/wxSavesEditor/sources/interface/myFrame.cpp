...
MyFrame::MyFrame(AppStartData& appStartData):
	...
	headersTexts{logs.error, getCampaignLocaleTextsFile("english", file::ONE_PLAY_SAVE_AND_LOAD), SAVE_NAME_FOCUSING_GAME_MAX},
...
void MyFrame::bindSkillLevelMenu(wxCommandEvent& event)
{
	...
	gameObjectsInterface.setValue(IndexSkillLevel, id - menuItemsID::SetSkillToEasy, onePlGame::SKILL_LEVEL_MAX);
}
...
void MyFrame::bindCampaignType(wxCommandEvent& event)
{
	...
	gameObjectsInterface.setValue(IndexCampaignType, id - menuItemsID::CampaignWithShop, onePlGame::CAMPAIGN_MAX);
}
...
void MyFrame::bindRacketSize(wxCommandEvent& event)
{
	...
	gameObjectsInterface.setValue(IndexRacketSize, id - menuItemsID::SmallRacket, onePlGame::RACKET_MAX );
}
...
void MyFrame::bindRimSize(wxCommandEvent& event)
{
	...
	gameObjectsInterface.setValue(IndexRimState, id - menuItemsID::RimNull + rims::RIM_NULL_STATE, rims::RIM_NULL_STATE, rims::RIM_MAX);
}

void MyFrame::bindCampaignHeaderMenu(wxCommandEvent& event)
{
	if( savesSlots.slot.gameType < onePlGame::CAMPAIGN_MAX )
	{
		try{
		...
		if( logWindow )
		{
			*logWindow << "Error: bad 'Campaign type' value. Should be equal to '" << onePlGame::CAMPAIGN_WITH_SHOP << "' or '" << onePlGame::CAMPAIGN_NO_SHOP << "' .\n";
		}
	}
}
