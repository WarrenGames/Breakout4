...
void GameObjectsInterface::setTextCtrlData(const GameSlotData& gameSlotData)
{
	eraseTextCtrlContent();
	*(count[IndexCampaignType])		<< static_cast<int>( gameSlotData.gameType );
...
void GameObjectsInterface::checkTextCtrl(GameSlotData& gameSlotData) const
{
	setSlotData(gameSlotData.gameType, count[IndexCampaignType] );
...
void GameObjectsInterface::setSizeSlotData(long unsigned& valueToSet, wxTextCtrl *textCtrl) const
{
	if( false == textCtrl->IsEmpty() )
	{
		valueToSet = std::stoul( textCtrl->GetLineText(0).ToStdString() );
