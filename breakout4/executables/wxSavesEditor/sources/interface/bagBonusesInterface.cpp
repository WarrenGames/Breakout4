...
void BagBonusesInterface::setTextCtrl(const GameSlotData& gameSlotData)
{
	eraseWeaponItemsTextCtrlContent();
	assert( gameSlotData.bonusesInBag.size() == bonusesCount.size() );
	for( size_t i{0} ; i < gameSlotData.bonusesInBag.size() && i < bonusesCount.size() ; ++i )
	{
		*(bonusesCount[i]) << static_cast<int>( gameSlotData.bonusesInBag[i] );
	}
}

void BagBonusesInterface::checkTextCtrl(GameSlotData& gameSlotData) const
{
