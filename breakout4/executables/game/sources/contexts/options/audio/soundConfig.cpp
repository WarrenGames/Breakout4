...
constexpr unsigned int VOL_GRADIENT_W = 8 * SQR_SIZE;
constexpr unsigned int VOL_GRADIENT_H = SQR_SIZE / 2;
...
SoundConfig::SoundConfig(Essentials& essentials, const sdl2::Font& font, const TextsBlocks& audioLangTexts):
	...
	soundsTxt{ essentials.logs, essentials.rndWnd, font, audioLangTexts[audioLang::SOUND], WHITE_COL, TexturePosition{2*SQR_SIZE, 3*SQR_SIZE, false, true} },
	enabled{ essentials.logs, essentials.rndWnd, font, audioLangTexts[audioLang::ENABLED], WHITE_COL, TexturePosition{4*SQR_SIZE, 3*SQR_SIZE, false, true} },
	disabled{ essentials.logs, essentials.rndWnd, font, audioLangTexts[audioLang::DISABLED], WHITE_COL, TexturePosition{4*SQR_SIZE, 3*SQR_SIZE, false, true} },
	volBar{essentials, {2*SQR_SIZE, 5*SQR_SIZE, VOL_GRADIENT_W, VOL_GRADIENT_H}, GREEN_COL, RED_COL, Offset{0, MIX_MAX_VOLUME}, MIX_MAX_VOLUME/2},
	playSound{essentials.logs, essentials.rndWnd, font, audioLangTexts[audioLang::PLAY_SOUND], GREEN_COL, MEDIUM_GRAY_COL, SQR_SIZE * 4, SQR_SIZE * 4, true},
	switchSoundBool{essentials.logs, essentials.rndWnd, font, audioLangTexts[audioLang::CHANGE], GREEN_COL, MEDIUM_GRAY_COL, SQR_SIZE*8, SQR_SIZE*3, true}
{
...
{
	if( std::ofstream soundConfiguration{ path::getConfigDirFile(essentials.prefPath, file::SOUND_CFG_FILE) } )
	{
		soundConfiguration << isSoundActive << " " << volBar.getVolume();
	}
	else{
		essentials.logs.error.wrFileOpeningError( path::getConfigDirFile(essentials.prefPath, file::SOUND_CFG_FILE), "save sound configuration." );
	}
}
...
{
	if( std::ifstream soundFile{ path::getConfigDirFile(essentials.prefPath, file::SOUND_CFG_FILE) } )
	{
		...
		if( soundFile >> isSoundActive >> soundVolume )
		{
			...
			essentials.logs.error.wrReadErrorMessage(path::getConfigDirFile(essentials.prefPath, file::SOUND_CFG_FILE), "'is sound active' boolean and 'sound volume'");
			isLoadingPerfect = false;
		}
	}
	else{
		essentials.logs.error.wrFileOpeningError( path::getConfigDirFile(essentials.prefPath, file::SOUND_CFG_FILE), "read sound configuration for the options.");
		isLoadingPerfect = false;
	}
}
