...
constexpr int BUTTONS_W_NUM = 16; //The number of buttons states in one row
constexpr int BUTTONS_H_NUM = 2;  //The number of buttons row : so it's 32 buttons max that are drawn.
constexpr double SINT16MAX = 32768;
...
JoystickValuesTextures::JoystickValuesTextures(AppLogFiles& logs, sdl2::RendererWindow& rndWnd, const sdl2::Font& arial, const TextsBlocks& joyLangTexts, const sdl2::JoystickEntity& joyEnt):
	hatsSquare{logs, rndWnd, WHITE_COL, SQR_SIZE*3, SQR_SIZE*3, TexturePosition{} },
	buttonOff{logs, rndWnd, WHITE_COL, SQR_SIZE/2, SQR_SIZE/2, TexturePosition{} },
	buttonPushed{logs, rndWnd, RED_COL, SQR_SIZE/2, SQR_SIZE/2, TexturePosition{} },
	axisGradientOff{logs, rndWnd, WHITE_COL, SQR_SIZE/2, SQR_SIZE*2, TexturePosition{} },
	axisGradientRed{logs, rndWnd, RED_COL, SQR_SIZE/2, SQR_SIZE, TexturePosition{GAME_SCREEN_WIDTH, GAME_SCREEN_HEIGHT} },
	buttonsText{logs, rndWnd, arial, joyLangTexts[joyStrings::BUTTON_NUM], BLACK_COL, TexturePosition{SQR_SIZE, SQR_SIZE * 2 - SQR_SIZE / 4, false, true} },
	hatsText{logs, rndWnd, arial, joyLangTexts[joyStrings::HAT_NUM], BLACK_COL, TexturePosition{SQR_SIZE, SQR_SIZE*5+SQR_SIZE/2, false, true} },
	axisText{logs, rndWnd, arial, joyLangTexts[joyStrings::AXIS_NUM], BLACK_COL, TexturePosition{SQR_SIZE, SQR_SIZE*9, false, false} },
...
void JoystickValuesTextures::drawButtonsStates(sdl2::RendererWindow& rndWnd)
{
	...
	for( int i{0} ; i < BUTTONS_H_NUM ; ++i )
	{
		for( int j{0} ; j < BUTTONS_W_NUM ; ++j )
		{
...
void JoystickValuesTextures::createNumbersTextures(AppLogFiles& logs, sdl2::RendererWindow& rndWnd, const sdl2::Font& arial)
{
	...
	{
		numbersTextures[i].texture.loadBlendedText(logs, rndWnd, arial, std::to_string(i), BLACK_COL);
