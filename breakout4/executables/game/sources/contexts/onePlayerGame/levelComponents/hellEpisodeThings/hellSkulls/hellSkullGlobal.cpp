#include "contexts/onePlayerGame/levelComponents/hellEpisodeThings/hellSkulls/hellSkullGlobal.h"
...
void onePlGame::HellSkullGlobal::update(ObjectMoving& ballMove, OnePlayerRacket& racket, const AllExplosions& explosions, const TextureCombo& ballTexture, const SoundHandler& sounds)
{
	skullPack.moveSkulls();
	skullPack.updateSkulls(drawer, ballMove, explosions, sounds);
	skullPack.withBallCollision(drawer, ballMove, ballTexture);
	skullPack.fireBallCollisionWithRacket(racket, sounds);
	activateSkullIfAny();
}
...
void onePlGame::HellSkullGlobal::drawFireBalls(Essentials& essentials, FireBallDrawer& fireBallDrawer)
{
	for( auto &skullPtr : skullPack )
	{
		if( skullPtr )
		{
			...
			{
				fireBallDrawer.drawFireBall(essentials, skullPtr->fireBallDrawData, skullPtr->fireBallMove);
			}
		}
	}
}
