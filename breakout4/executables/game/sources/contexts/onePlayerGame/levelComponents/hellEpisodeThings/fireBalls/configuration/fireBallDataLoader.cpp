...
{
	if( playerData.getLevelType() == onePlGame::EPISODE_HELL )
	{
...
void onePlGame::FireBallDataLoader::readLinesOfConfigFile(Essentials& essentials, const PlayerData& playerData, std::ifstream& fireBallFile)
{
	...
	while( std::getline( fireBallFile, fileLine ) )
	{
		...
		if( lineStream >> data.id >> data.behaviorId >> data.skillBits >> data.sqrSizeRelativePos >> data.totalWaitTime >> data.randomTimeInterval >> data.seedAdd )
		{
