...
void onePlGame::FireBallGlobal::drawFireBalls(Essentials& essentials)
{
	for( auto const &element : bottomFireBalls )
	{
		if( element )
		{
			if( element->isActive )
			{
				fireBallDrawer.drawFireBall(essentials, element->drawData, element->move );
			}
		}
	}
}

void onePlGame::FireBallGlobal::updateFireBalls(OnePlayerRacket& racket, const SoundHandler& sounds)
{
	bottomFireBalls.update(racket, sounds);
	bottomFireBalls.startActivity(sounds);
}
