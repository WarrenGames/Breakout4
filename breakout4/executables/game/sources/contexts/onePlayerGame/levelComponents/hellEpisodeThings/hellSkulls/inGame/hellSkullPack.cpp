...
void onePlGame::HellSkullPack::populateVector(AppLogFiles& logs, const PlayerData& playerData, const FireBallsPaths& fireBallsPaths, const HellSkullDataLoader& dataLoader)
{
	...
	if( fireBallsPaths.paths.getMovesCount() == hell::FB_PATH_MAX )
	{
		...
		isLoadingPerfect = false;
		logs.error << "Error: the fire balls paths data size is wrong; expected: " << hell::FB_PATH_MAX << " whereas the real size is: " << fireBallsPaths.paths.getMovesCount() << " .\n";
	}
}
...
void onePlGame::HellSkullPack::updateSkulls(const HellSkullDrawer& skullDrawer, const spriteMove::ObjectMoving& ballMove, const AllExplosions& explosions, const SoundHandler& sounds)
{
