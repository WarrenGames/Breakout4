...
BonusesCountGraphics::BonusesCountGraphics(Essentials& essentials):
	...
	bonusesCount(onePlGame::BONUS_MAX),
	updateTexts(onePlGame::BONUS_MAX, true)
{	
...
{
	assert( bonusNumber < onePlGame::BONUS_MAX );
...
void BonusesCountGraphics::updateNeededTexts(Essentials& essentials, const PlayerData& playerData, const std::vector<TextureCombo>& bagIcons)
{
	if( fancyFont )
	{
		for( std::size_t i{0} ; i < onePlGame::BONUS_MAX ; ++i )
		{
			...
			if( updateTexts[i] )
			{
				bonusesCount[i].texture.loadBlendedText(essentials.logs, essentials.rndWnd, fancyFont, std::to_string(playerData.bonusesInBag[i]), WHITE_COL);
