...
constexpr int RACKET_COLLIDE_CORNER = 20;
...
BallThings::BallThings(AppLogFiles& logs, sdl2::RendererWindow& rndWnd, std::size_t monstersNumber):
	...
	move{ AccurCoords{GAME_SCREEN_WIDTH/2, GAME_SCREEN_HEIGHT - SQR_SIZE * 2}, 0, Offset{1,-1} },
	texture{ logs, rndWnd, "textures/sprites/balls/yellowBall.png" },
	speeds{logs, "data/levels/1player/options/ballSpeeds.cfg"}
{
	if( !( texture.texture && speeds.isLoadingPerfect ) )
...
{
	if( moveBallDelay.hasTimeElapsed( std::chrono::microseconds{onePlGame::MICRO_SEC_REFRESH_TIME_RATE} ) && isMainBallActive )
	{
		move.moveObject();
...
void BallThings::determineBallBounceWithRacket(const SDL_Rect& racketRect, const SoundHandler& sounds)
{
	if( move.get_y_position() + texture.sprite.height() / 2 < racketRect.y + racketRect.h )
	{
		...
		sounds.playSound(onePlGame::SND_COL_RACK_W_BALL);
...
{
	if( move.get_x_position() + texture.sprite.width() / 2 < racketRect.x + RACKET_COLLIDE_CORNER )
	{
		sounds.playSound(onePlGame::SND_COL_RACK_W_BALL);
		giveObjectNewDirection(move, Offset{-1, -1} );
	}
	else if( move.get_x_position() - texture.sprite.width() / 2 >= racketRect.x + racketRect.w - RACKET_COLLIDE_CORNER )
	{
		sounds.playSound(onePlGame::SND_COL_RACK_W_BALL);
		giveObjectNewDirection(move, Offset{1, -1} );
	}
...
{
	assert( skillLevel < speeds.speeds.size() );
	if( move.getSpeed() < speeds.speeds[skillLevel] )
	{
...
{	
	if( canTestRimCollisions )
	{
		...
		{
			rectCollision::startCollisionTest(move, SpriteSize{texture.sprite.width(), texture.sprite.height()}, rect, rectCollision::BALL_INTERVAL, sounds, onePlGame::SND_COL_BALL_W_BACKGRND);
		}
