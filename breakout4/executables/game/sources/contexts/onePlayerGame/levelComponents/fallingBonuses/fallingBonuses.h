...
constexpr double FALL_BONUSES_SPEED = 2;

struct FallingBonusData
{
	std::size_t bonusType;
	ObjectMoving objectMove;
...
public:
	...
	void detectCollisionWithRacket(PlayerData& playerData, OnePlayerRacket& racket, BallThings& ball, OnePlayerRims& rims, InfoBar& infoBar, const SoundHandler& sounds);

private:
	void applyBonusModificationToPlayer(std::unique_ptr< FallingBonusData >& bonus, PlayerData& playerData, OnePlayerRacket& racket, BallThings& ball, OnePlayerRims& rims, const SoundHandler& sounds);
	void improveBall(std::unique_ptr< FallingBonusData >& bonus, BallThings& ball, const SoundHandler& sounds);
	void speedUpRacket(std::unique_ptr< FallingBonusData >& bonus, PlayerData& playerData, OnePlayerRacket& racket, const SoundHandler& sounds);
	...
	void enlargeRims(std::unique_ptr< FallingBonusData >& bonus, OnePlayerRims& rims, const SoundHandler& sounds);
	void shrinkRims(std::unique_ptr< FallingBonusData >& bonus, OnePlayerRims& rims, const SoundHandler& sounds);
	void enlargeRacket(std::unique_ptr< FallingBonusData >& bonus, OnePlayerRacket& racket, const SoundHandler& sounds);
