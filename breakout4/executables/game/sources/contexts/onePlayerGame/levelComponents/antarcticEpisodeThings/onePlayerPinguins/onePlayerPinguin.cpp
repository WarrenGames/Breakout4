...
constexpr char PINGUIN_REVOKE_FILE[] = "data/levels/1player/options/pinguinRevoke.cfg";
...
void OnePlayerPinguin::readSingleLine(AppLogFiles& logs, const std::string& levelDataFilePath, std::istringstream& lineStream, unsigned lineIndex)
{
	if( lineStream >> lineIndex )
	{
		switch( lineIndex )
		{
			case pinguin::fileIndex::NEW_PINGUIN:
...
{
	if( std::ifstream pinguinTolerance{PINGUIN_REVOKE_FILE} )
	{
		std::vector<unsigned> hits(onePlGame::SKILL_LEVEL_MAX);
		for( auto &number : hits )
		{
			if( !( pinguinTolerance >> number ) )
			{
				isLoadingPerfect = false;
				logs.error << "Error: reading data in '" << PINGUIN_REVOKE_FILE << "' file failed (probably there not enough data; " 
						 << onePlGame::SKILL_LEVEL_MAX << " of them are needed.\n";
			}
		}
		...
		isLoadingPerfect = false;
		logs.error << "Error: couldn't load '" << PINGUIN_REVOKE_FILE << "' in order to load data relative to pinguin tolerance.\n";
	}
}
...
void OnePlayerPinguin::activeOnePinguin()
{
	...
	{
		auto iter = std::find_if(pinguins.begin(), pinguins.end(), [](auto &ping){return PINGUIN_IS_INACTIVE == ping.getState(); } );
...
void OnePlayerPinguin::updateMove()
{
	if( moveTime.hasTimeElapsed( std::chrono::milliseconds{10} ) )
	{
		if( false == pinguins.empty() )
		{
			...
			{
				if( PINGUIN_IS_INACTIVE != ping.getState() )
				{
					ping.movePinguin();
				}
			}
...
void OnePlayerPinguin::browseForCollisions(const SDL_Rect& collisionRect, const MatrixTemp2D<BrickData>& grid)
{
	if( false == pinguins.empty() )
	{
		for( auto &ping : pinguins )
		{
			if( PINGUIN_IS_ACTIVE == ping.getState() || PINGUIN_START_MIND_PLAYER == ping.getState() )
			{
...
void OnePlayerPinguin::resetPinguinStatusIfOffscreen()
{
	for( auto &ping : pinguins )
	{
		if( false == ping.isPinguinInsideScreen(collisionRect ) && PINGUIN_IS_LEAVING == ping.getState() )
		{
			ping.setState(PINGUIN_IS_INACTIVE);
		}
	}
}