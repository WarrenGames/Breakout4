...
bool ballCol::isThereAnElementInGrid(LevelComponents& comp, const Offset& coords)
{
	...
	{
		return comp.grid(coords.x, coords.y).index != BRICKS::INDEX::NO_BRICK;
	}
	else
		return false;
}
...
void ballCol::testLeftCollision(LevelComponents& comp, BallThings& ball, PlayerData& playerData)
{
		...
		&& ballCol::isThereAnElementInGrid(comp, ballCol::getBrickCoordinates(ball.gridRelPos, position::LEFT) ) )
	{
		...
		comp.infoBar.setUpdateFlag(infoBarTexts::PLAYER_SCORE);
		comp.infoBar.setUpdateFlag(infoBarTexts::BONUS_COINS_NUM);
	}
}
...
void ballCol::testTopLeftCollision(LevelComponents& comp, BallThings& ball, PlayerData& playerData)
{
	if( ball.move.get_x_norm() < 0
		...
		&& ball.move.get_x_position() % SQR_SIZE < ball.texture.sprite.width() / 2 - onePlGame::BALL_CORNER_INTERVAL 
		&& ball.move.get_y_position() % SQR_SIZE < ball.texture.sprite.height() / 2 - onePlGame::BALL_CORNER_INTERVAL 
...
void ballCol::testTopRightCollision(LevelComponents& comp, BallThings& ball, PlayerData& playerData)
{
	if( ball.move.get_x_norm() > 0 
		&& ball.move.get_y_norm() < 0 
		&& ball.move.get_x_position() % SQR_SIZE > SQR_SIZE - ball.texture.sprite.width() / 2 + onePlGame::BALL_CORNER_INTERVAL 
		&& ball.move.get_y_position() % SQR_SIZE < ball.texture.sprite.height() / 2  - onePlGame::BALL_CORNER_INTERVAL 
...
void ballCol::testBottomLeftCollision(LevelComponents& comp, BallThings& ball, PlayerData& playerData)
{
	if( ball.move.get_x_norm() < 0 
		&& ball.move.get_y_norm() > 0
		&& ball.move.get_x_position() % SQR_SIZE < ball.texture.sprite.width() / 2 - onePlGame::BALL_CORNER_INTERVAL 
		&& ball.move.get_y_position() % SQR_SIZE > SQR_SIZE - ball.texture.sprite.height() / 2 + onePlGame::BALL_CORNER_INTERVAL 
...
{
	if( ball.move.get_x_position() > GAME_SCREEN_WIDTH - ball.texture.sprite.width() / 2 
		&& ball.move.get_x_norm() > 0 )
	{
		comp.sounds.playSound(onePlGame::SND_COL_BALL_W_BACKGRND);
		ball.move.reverse_x_vector();
...
{
	if( ball.move.get_y_position() < onePlGame::INFO_BOARD_Y_MARGIN * SQR_SIZE + ball.texture.sprite.height() / 2 
		&& ball.move.get_y_norm() < 0 )
	{
		comp.sounds.playSound(onePlGame::SND_COL_BALL_W_BACKGRND);
		ball.move.reverse_y_vector();
...
{
	if( ball.move.get_y_position() > GAME_SCREEN_HEIGHT - ball.texture.sprite.height() / 2 
		&& ball.move.get_y_norm() > 0 )
	{
		comp.sounds.playSound(onePlGame::SND_COL_BALL_W_BACKGRND);
		ball.move.reverse_y_vector();
