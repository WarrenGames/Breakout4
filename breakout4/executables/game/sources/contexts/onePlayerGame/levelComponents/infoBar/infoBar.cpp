...
InfoBar::InfoBar(Essentials& essentials):
	...
	isGraphicalTextUpdateNeeded{ true, true, true, true }
{
...
void InfoBar::updateDataTexts(Essentials& essentials, const PlayerData& playerData, const RacketData& racketData)
{
	...
	updateText(essentials, racketData.getSpeed(), infoBarTexts::RACKET_SPEED, BLUE_COL, Offset{blackSquares[infoBarTexts::RACKET_SPEED].sprite.x_pos() + SQR_SIZE/4, SQR_SIZE*3/4});
...
void InfoBar::createGraphicalBarTexts(Essentials& essentials)
{
	if( fancyFont )
	{
		...
		{
			std::array< Offset, infoBarTexts::MAX > textsPositions{ Offset{ SQR_SIZE, SQR_SIZE/4}, 
			...
			for( std::size_t i{0} ; i < infoBarTexts::MAX ; ++i )
			{
				...
				barTexts[i].sprite.setTexturePosition( TexturePosition{textsPositions[i].x, textsPositions[i].y, false, true} );
