...
LevelInputs::LevelInputs(Essentials& essentials):
	...
	functionsStates{ false, false, false, false, false },
	crossMove{ 0, 0 },
	escapeState{ false },
	windowClosing{ false },
	mousePosition{ 0, 0},
...
{
	if( inputsData.joystickData.joyMoveType == onePlGame::JOYMOV_AXES && static_cast<unsigned>(event.jaxis.axis) == inputsData.joystickData.axisOrHatNum )
	{
		if( event.jaxis.value >= static_cast<Sint32>(inputsData.joystickData.axisMoveThreshold) )
		{
			functionsStates[onePlGame::INP_MOVE_LEFT] = false;
			functionsStates[onePlGame::INP_MOVE_RIGHT] = true;
		}
		else if( event.jaxis.value <= static_cast<Sint32>(-inputsData.joystickData.axisMoveThreshold) )
		{
			functionsStates[onePlGame::INP_MOVE_LEFT] = true;
			functionsStates[onePlGame::INP_MOVE_RIGHT] = false;
		}
		else{
			functionsStates[onePlGame::INP_MOVE_LEFT] = false;
			functionsStates[onePlGame::INP_MOVE_RIGHT] = false;
...
{
	if( inputsData.joystickData.joyMoveType == onePlGame::JOYMOV_HATS && static_cast<unsigned>(event.jhat.hat) == inputsData.joystickData.axisOrHatNum )
	{
...
{
	setInputFunctionWithJoyButton(event.jbutton.button, onePlGame::JOYBTN_BALL_TRACE, onePlGame::INP_BALL_TRACE, valueToAppend);
	setInputFunctionWithJoyButton(event.jbutton.button, onePlGame::JOYBTN_LOOK_BAG, onePlGame::INP_LOOK_BAG, valueToAppend);
	setInputFunctionWithJoyButton(event.jbutton.button, onePlGame::JOYBTN_SWITCH, onePlGame::INP_SWITCH, valueToAppend);
}
...
{
	if( inputsData.joystickData.crossMoveType == onePlGame::JOYMOV_HATS ) 
	{
...
{
	if( inputsData.joystickData.crossMoveType == onePlGame::JOYMOV_AXES )
	{
...
{
	if( event.jaxis.axis == inputsData.joystickData.crossNum[onePlGame::JOY_CROSS_MOVE_X] )
	{
		if( event.jaxis.value >= static_cast<Sint32>(inputsData.joystickData.axisMoveThreshold) )
		{
			crossMove.x = 1;
...
{
	if( event.jaxis.axis == inputsData.joystickData.crossNum[onePlGame::JOY_CROSS_MOVE_Y] )
	{
		if( event.jaxis.value >= static_cast<Sint32>(inputsData.joystickData.axisMoveThreshold) )
		{
			crossMove.y -= 1;
