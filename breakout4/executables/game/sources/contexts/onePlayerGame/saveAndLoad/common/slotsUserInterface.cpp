...
SlotsUserInterface::SlotsUserInterface(Essentials& essentials, unsigned contextFocus):
	localeTexts{essentials.logs.error, path::getLanguageFile(essentials.language, file::ONE_PLAY_SAVE_AND_LOAD), SAVE_NAME_FOCUSING_GAME_MAX},
	arial{essentials.logs.error, ARIALFONTPATH, FONT_SMALL_PTSIZE},
	fancyFont{essentials.logs.error, FANCYFONTPATH, FONT_MEDIUM_PTSIZE},
	quitButton{essentials.logs, essentials.rndWnd, fancyFont, localeTexts[SAVE_NAME_QUIT_MENU], GREEN_COL, MEDIUM_GRAY_COL, GAME_SCREEN_WIDTH - SQR_SIZE * 2, SQR_SIZE, true},
	slotsData(SLOTS_NUMBER)
{
...
void SlotsUserInterface::saveToFile(Essentials& essentials, const PlayerData& playerData)
{
	...
	{
		for( unsigned slotIndex{0} ; slotIndex < SLOTS_NUMBER ; ++slotIndex )
		{
...
{
	assert( SAVE_NAME_FOCUSING_ON_SAVING == contextFocus || SAVE_NAME_FOCUSING_ON_LOADING == contextFocus );
	if( SAVE_NAME_FOCUSING_ON_SAVING == contextFocus )
	{
		title.texture.loadBlendedText(essentials.logs, essentials.rndWnd, fancyFont, localeTexts[SAVE_NAME_FOCUSING_ON_SAVING], WHITE_COL);
		title.resetSpritePosition(TexturePosition{ GAME_SCREEN_WIDTH / 2, SQR_SIZE, true, true } );
	}
	else if( SAVE_NAME_FOCUSING_ON_LOADING == contextFocus )
	{
		title.texture.loadBlendedText(essentials.logs, essentials.rndWnd, fancyFont, localeTexts[SAVE_NAME_FOCUSING_ON_LOADING], WHITE_COL); 
		title.resetSpritePosition(TexturePosition{GAME_SCREEN_WIDTH / 2, SQR_SIZE, true, true} );
	}
}
...
void gameSaving::startSaving(AppLogFiles& logs, const PrefPathFinder& prefPath, const GameSlotData& slot, unsigned gameSaveNumber)
{
	...
	if( ! saveFile )
	{
		throw WriteError{"Error: unable to open '" + path::getGameSaveFullPath(prefPath, gameSaveNumber) 
...
void gameLoading::startLoading(AppLogFiles& logs, const PrefPathFinder& prefPath, GameSlotData& slot, unsigned gameSaveNumber)
{
	...
	if( ! gameSave )
	{
		throw ReadError{"Error: unable to open '" + path::getGameSaveFullPath(prefPath, gameSaveNumber) 
