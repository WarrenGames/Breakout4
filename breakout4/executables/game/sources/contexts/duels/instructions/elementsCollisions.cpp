...
{
	assert( coinA_num < duels::COIN_MAX && coinB_num < duels::COIN_MAX);
	...
	if( false == duels::isThisTheSameArrayElement(coinA_num, coinB_num) && coinA.collision.isEffective && coinB.collision.isEffective )
	{
		...
		{
			if( false == bouncing::objectsCollide(coinA.move, coinRect, coinB.move, coinRect, duels::SIZE_DECAL_COLLIDE_BOX) )
			{
...
{
	assert( coin_num < duels::COIN_MAX && ball_num < duels::BALL_MAX );
	...
	if( coin.collision.isEffective && ball.collision.isEffective )
	{
		...
		{
			if( false == bouncing::objectsCollide(coin.move, coinSize, ball.move, ballSize, duels::SIZE_DECAL_COLLIDE_BOX) )
			{
...
{
	assert( ballA_num < duels::BALL_MAX && ballB_num < duels::BALL_MAX );
	...
	if( false == duels::isThisTheSameArrayElement(ballA_num, ballB_num) && ballA.collision.isEffective && ballB.collision.isEffective )
	{
		...
		{
			if( false == bouncing::objectsCollide(ballA.move, ballRect, ballB.move, ballRect, duels::SIZE_DECAL_COLLIDE_BOX) )
			{
