...
{
	if( move.get_x_norm() < 0 && move.get_x_position() - COIN_TEXTURE_SIZE / 2 <= 0 ) 
	{
...
{
	if( move.get_x_norm() > 0 && move.get_x_position() + COIN_TEXTURE_SIZE / 2 > GAME_SCREEN_WIDTH )
	{
		move.reverse_x_vector();
		move.adjust_x_speed();
	}
...
{
	if( animIndex < duels::COIN_FRAMES_NUMBER - 1 )
		animIndex++;
	else
		animIndex = 0;
}
