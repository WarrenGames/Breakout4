...
private:
	std::array< duels::ComboCoinData, duels::COIN_MAX > coinsData;
...
public:
	...
	void update(const MatrixTemp2D<BrickData>& levelGrid, BallsThings& balls, IntRandomGenerator& randomGen, const SoundHandler& commonSounds);
	void animateCoins();
	void moveCoins();
	...
	void createNewCoinIfPossible(BallsThings& balls, IntRandomGenerator& randomGen, const SoundHandler& commonSounds);
	void setCanCreateCoin();
	
	void drawBonusCoins(sdl2::RendererWindow& rndWnd);
	
	void checkCollisionWithRect(const SDL_Rect& externObjectRect, std::array< bool, duels::COIN_MAX >& canCollide);
};
