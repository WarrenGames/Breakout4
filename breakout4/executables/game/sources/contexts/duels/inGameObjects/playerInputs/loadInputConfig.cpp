...
{
	assert( playerNum < PLAYER_MAX );
	JoystickFileData joyFileData{};
	std::array<SDL_Keycode, KEY_MAX> keybData { SDLK_UNKNOWN, SDLK_UNKNOWN, SDLK_UNKNOWN, SDLK_UNKNOWN};
	unsigned deviceType{DEVICE_KEYBOARD};
...
bool duels::areInputRessourcesAllocated(AppLogFiles& logs, unsigned deviceType, const std::array<SDL_Keycode, KEY_MAX>& keybData, const JoystickFileData& joyFileData, 
										std::unique_ptr<InputEntity>& inputPtr)
{
	inputPtr.reset( new InputEntity{deviceType, keybData, joyFileData} );
	if( inputPtr )
	{
		logs.error << "  The players input data was perfectly loaded in configurations files.\n";
		return true;
	}
	else{
...
bool duels::loadKeyboardData(AppLogFiles& logs, const PrefPathFinder& prefPath, unsigned playerNum, std::array<SDL_Keycode, KEY_MAX>& keybData)
{
...
bool duels::areKeyboardDataRead(AppLogFiles& logs, const PrefPathFinder& prefPath, unsigned playerNum, std::array<SDL_Keycode, KEY_MAX>& keybData, std::ifstream& keybFile)
{
