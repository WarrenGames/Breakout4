...
void duels::BallsThings::update(const MatrixTemp2D<BrickData>& levelGrid, duels::LevelOptions& levelOptions, CoinsThings& coins, IntRandomGenerator& randomGen, const SoundHandler& commonSounds, 
								std::array< duels::ScoreDraw, duels::PLAYER_MAX>& scoreDrawing, unsigned& givenPoints)
{
	moveBalls();
...
void duels::BallsThings::checkBallsPositionsForScore(std::array< int, duels::PLAYER_MAX>& playersScores, std::array< duels::ScoreDraw, duels::PLAYER_MAX>& scoreDrawing, unsigned& givenPoints)
{
	if( canCheckForScores )
	{
		giveBluePlayerAPoint(playersScores[duels::PLAYER_BLUE], scoreDrawing[duels::PLAYER_BLUE], givenPoints);
		giveRedPlayerAPoint(playersScores[duels::PLAYER_RED], scoreDrawing[duels::PLAYER_RED], givenPoints);
...
void duels::BallsThings::giveRedPlayerAPoint(int& redPlayerScore, duels::ScoreDraw& redScore, unsigned& givenPoints)
{
	for( auto &ball : ballsData )
	{
		if( ball.collision.isEffective )
		{
			if( ball.move.get_y_position() > GAME_SCREEN_HEIGHT + SQR_SIZE / 2)
			{
				redPlayerScore++;
				ball.collision.isEffective = false;
...
void duels::BallsThings::disableCoinIfOutsideScreen()
{
	for( auto &ball : ballsData )
	{
		if( ball.collision.isEffective )
		{
			if( ball.move.get_x_position() < -SQR_SIZE || ball.move.get_x_position() > GAME_SCREEN_WIDTH + SQR_SIZE )
			{
				ball.collision.isEffective = false;
			}
		}
	...
	if( collisionBools.canCheckCollisionWithGrid )
	{
		for( auto &ball : ballsData )
		{
			if( ball.collision.isEffective)
			{
				...
				testGridCollisions(levelGrid, ball.move, SpriteSize{BALL_TEXTURE_SIZE, BALL_TEXTURE_SIZE}, wasThereCol);
...
void duels::BallsThings::checkCollisionWithRect(const SDL_Rect& externObjectRect, std::array< bool, duels::BALL_MAX >& canCollide)
{
	for( std::size_t ballNum{0} ; ballNum < duels::BALL_MAX ; ++ballNum )
	{
...
void duels::BallsThings::playSoundIfCollision(const SoundHandler& sounds, bool wasThereCol)
{
	if( wasThereCol )
	{
		sounds.playSound(duels::SND_COL_BALL_WBG);
	}
}