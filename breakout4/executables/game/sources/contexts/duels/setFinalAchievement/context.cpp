...
void duels::finAchiev::mainContext(Essentials& essentials)
{
	if( const TextsBlocks texts{essentials.logs.error, path::getLanguageFile(essentials.language, file::DUELS_ACHIEVEMENTS), duels::finAchiev::MAX } )
	{
		if( const sdl2::Font fancyFont{essentials.logs.error, FANCYFONTPATH, FONT_BIG_PTSIZE} )
		{
			...
			const TextureCombo gradientBg{essentials.logs, essentials.rndWnd, SDL_Rect{ 0, 0, GAME_SCREEN_WIDTH, GAME_SCREEN_HEIGHT}, WHITE_COL, {0, 128, 255, 255}, GRAD_TEX_ON_Y};
