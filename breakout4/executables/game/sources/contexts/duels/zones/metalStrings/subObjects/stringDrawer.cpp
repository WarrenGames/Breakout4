#include "contexts/duels/zones/metalStrings/subObjects/stringDrawer.h"
...
constexpr char HORIZONTAL_TEXTURE_PREFIX[] = "textures/gameZones/factory/horizontal_string/str_h_";
constexpr char VERTICAL_TEXTURE_PREFIX[] = "textures/gameZones/factory/vertical_string/str_v_";

metalStrings::StringsDrawer::StringsDrawer(Essentials& essentials, const std::string& filePath):
	data( duels::PLAYER_MAX ),
	textures( metalStrings::ORIENT_MAX )
{
	readDataLevelFile(essentials, filePath);
	loadStringsTextures(essentials, metalStrings::ORIENT_HORIZ, HORIZONTAL_TEXTURE_PREFIX);
	loadStringsTextures(essentials, metalStrings::ORIENT_VERT, VERTICAL_TEXTURE_PREFIX);
}
...
{
	assert( playerNumber < duels::PLAYER_MAX );
	assert( directionType <= metalStrings::DIR_STOP );
...
{
	assert( stringData.orientation < metalStrings::ORIENT_MAX );
	assert( stringData.textureIndex < metalStrings::STRING_FRAMES_NUM );
	
	switch( stringData.orientation )
	{
		case metalStrings::ORIENT_HORIZ:
...
void metalStrings::StringsDrawer::readLine(Essentials& essentials, std::istringstream& lineStream, std::size_t fileLineNumber, const std::string& filePath)
{
	...
	unsigned orientation{metalStrings::ORIENT_MAX};
	unsigned forwardDir{metalStrings::DIR_STOP};
	unsigned backwardDir{metalStrings::DIR_STOP};
...
void metalStrings::StringsDrawer::loadStringsTextures(Essentials& essentials, std::size_t orientation, const std::string& texturePrefix)
{
	...
	for( std::size_t i{0} ; i < metalStrings::STRING_FRAMES_NUM ; ++i )
	{
