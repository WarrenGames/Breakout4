...
{
	for(std::size_t i{0} ; i < BUBBLES_CLUSTER_NUMBER ; ++i )
	{
...
void BubblesDrawer::animFrame(std::size_t index)
{
	...
	if( frameIndex[index] + 1 < BUBBLES_FRAME_NUMBER )
		frameIndex[index]++;
	else
		frameIndex[index] = 0;
}