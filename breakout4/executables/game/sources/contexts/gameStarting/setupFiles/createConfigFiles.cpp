...
void cfgFiles::createDrawFrequencyFile(LogFile& log, const PrefPathFinder& prefPath)
{
	...
	{
		if( FS_NEW_CREATED == createFs::mkNewFileIfMissing(path::getConfigDirFile(prefPath, file::DRAW_FREQ_FILE) ) )
		{
...
void cfgFiles::createSoundConfigFile(LogFile& log, const PrefPathFinder& prefPath)
{
	if( prefPath.isAValidPathFound() )
	{
		if( FS_NEW_CREATED == createFs::mkNewFileIfMissing(path::getConfigDirFile(prefPath, file::SOUND_CFG_FILE) ) )
		{
...
void cfgFiles::createMusicConfigFile(LogFile& log, const PrefPathFinder& prefPath)
{
	if( prefPath.isAValidPathFound() )
	{
		if( FS_NEW_CREATED == createFs::mkNewFileIfMissing(path::getConfigDirFile(prefPath, file::MUSIC_CFG_FILE) ) )
		{
...
{
	for( unsigned skill{0} ; skill < onePlGame::SKILL_LEVEL_MAX ; ++skill )
	{
		if( FS_NEW_ERROR == createFs::mkNewFileIfMissing( path::getShopRaceBestScoresFilePath(prefPath, skill) ) )
		{
			log.wrFileOpeningError(path::getShopRaceBestScoresFilePath(prefPath, skill), "write 'shop race' one player scores");
		}
		if( FS_NEW_ERROR == createFs::mkNewFileIfMissing( path::getStdRaceBestScoresFilePath(prefPath, skill) ) )
		{
