...
{
	TextureCombo sdlLogo{essentials.logs, essentials.rndWnd, "textures/gameGUI/sdlLogo/sdlLogo1280.png", TexturePosition{GAME_SCREEN_WIDTH / 2, GAME_SCREEN_HEIGHT / 2, true, true} };
...
void logoDisplay::resizeLogoIfNeeded(Essentials& essentials, TextureCombo& sdlLogo)
{
	if( sdlLogo.sprite.width() > essentials.rndWnd.width() || sdlLogo.sprite.height() > essentials.rndWnd.height() )
	{
		...
		sdlLogo.sprite.setTexturePosition( TexturePosition{GAME_SCREEN_WIDTH / 2, GAME_SCREEN_HEIGHT / 2, true, true} );
	}
}