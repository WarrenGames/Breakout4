...
{
	if( canCollideWithGridTop(move, size, duels::BALL_CORNER_INTERVAL) && canCollideWithGridLeft(move, size, duels::BALL_CORNER_INTERVAL)
...
{
	if( canCollideWithGridTop(move, size, duels::BALL_CORNER_INTERVAL) && canCollideWithGridRight(move, size, duels::BALL_CORNER_INTERVAL) 
		&& isThereAnElementAtThisPosition(levelGrid, move, Offset{1, -1}) && !( move.get_x_norm() < 0 && move.get_y_norm() > 0 ) )
	{
		giveObjectNewDirection(move, Offset{-1, 1});
		wasThereCol = true;
...
{
	if( canCollideWithGridBottom(move, size, duels::BALL_CORNER_INTERVAL) && canCollideWithGridLeft(move, size, duels::BALL_CORNER_INTERVAL) 
		&& isThereAnElementAtThisPosition(levelGrid, move, Offset{-1, 1}) && !( move.get_x_norm() > 0 && move.get_y_norm() < 0 ) )
	{
		giveObjectNewDirection(move, Offset{1, -1});
		wasThereCol = true;
...
{
	if( canCollideWithGridBottom(move, size, duels::BALL_CORNER_INTERVAL) && canCollideWithGridRight(move, size, duels::BALL_CORNER_INTERVAL) 
		&& isThereAnElementAtThisPosition(levelGrid, move, Offset{1, 1}) && !( move.get_x_norm() < 0 && move.get_y_norm() < 0 ) )
	{
		giveObjectNewDirection(move, Offset{-1, -1});
		wasThereCol = true;
