...
{
	if( state == PINGUIN_IS_ACTIVE || state == PINGUIN_START_MIND_PLAYER )
...
void PinguinBehaviorData::quackFromItsLeft(const SDL_Rect& racketRect, bool playerRightState, const SoundHandler& sounds)
{
	...
	{
		sounds.playSound(onePlGame::SND_QUACK);
		canPinguinQuack = false;
		anger++;
	}
}
...
{
	if( PINGUIN_IS_ACTIVE == state )
	{
		rectCollision::startCollisionTest(drawingData.move, SpriteSize{pinguinRect.w, pinguinRect.h}, rimRect, SQR_SIZE / 8, sounds, onePlGame::SND_COL_BALL_W_BACKGRND);
	}
}
...
void PinguinBehaviorData::startMindPlayer(const SDL_Rect& racketRect, const SDL_Rect& pinguinRect)
{
	...
	{
		giveObjectNewDirection(drawingData.move, Offset{0, 1});
		state = PINGUIN_START_MIND_PLAYER;
	}
}
...
{
	return drawingData.move.get_y_position() >= GAME_SCREEN_HEIGHT - SQR_SIZE - SQR_SIZE / 4 
		&& drawingData.move.get_y_position() < GAME_SCREEN_HEIGHT - SQR_SIZE + SQR_SIZE / 4;
}

void PinguinBehaviorData::makeStayOnRacketLevelIfMindPlayer(const SDL_Rect& racketRect)
{
	if( PINGUIN_START_MIND_PLAYER == state && isPinguinOnSameLevelAsRacket() && 0 == drawingData.move.get_x_norm() && 1 == drawingData.move.get_y_norm() )
	{
...
bool PinguinBehaviorData::isPinguinInsideScreen(const SDL_Rect& pinguinRect) const
{
	return drawingData.move.get_x_position() + pinguinRect.w / 2 >= 0
		&& drawingData.move.get_x_position() - pinguinRect.w / 2 < GAME_SCREEN_WIDTH
		&& drawingData.move.get_y_position() + pinguinRect.h / 2 >= 0
		&& drawingData.move.get_y_position() - pinguinRect.h / 2 < GAME_SCREEN_HEIGHT;
}