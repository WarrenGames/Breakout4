...
StalactiteData::StalactiteData(const Offset& position):
	...
	trembleIndex{0},
	direction{stalactites::DEFAULT_DIRECTION},
	rumbleCount{0},
	state{ stalactites::STATE_STATIC },
	length{ stalactites::DEFAULT_LENGTH }
{
...
{
	if( stalactites::STATE_RUMBLE == state )
	{
...
void StalactiteData::startFallingIfRumbleEnds()
{
	if( stalactites::STATE_RUMBLE == state )
	{
		if( stalactites::RUMBLE_MAX <= rumbleCount )
		{
			state = stalactites::STATE_FALLING;
			move.setSpeed(stalactites::FALL_SPEED);
			move.setBothVectors( getDirectionVectors(stalactites::DIRECTIONS_NUMBER, direction, SQR_SIZE) );
			move.adjust_x_speed();
			move.adjust_y_speed();
			rumbleCount = 0;
		}
...
{
	if( stalactites::STATE_FALLING == state )
		move.moveObject();
}
