...
constexpr char EXPLO_CONFIG_FILE_PATH[] = "data/explosions/explosions";
constexpr char EXPLO_TEXTURE_DIR[] = "textures/sprites/explosions/";
...
{
	if( std::ifstream mainReferenceExplosionFile{EXPLO_CONFIG_FILE_PATH} )
	{
	...
	{
		logs.error.wrFileOpeningError(EXPLO_CONFIG_FILE_PATH, "load explosions textures");
...
void ExplosionsLoader::loadEntityExplosion(AppLogFiles& logs, sdl2::RendererWindow& rndWnd, const std::string& referenceLine)
{
	...
	{
		explosionsTextures.emplace_back( ExplosionChunk{logs, rndWnd, imagesNumber, EXPLO_TEXTURE_DIR + filePrefix + "_", frameDuration, zenithImage} );
	}
}
