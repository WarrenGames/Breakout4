...
BoxEvents::BoxEvents():
	numbersState(BOX_EV_MAX, false),
	specificKeys(BOX_EV_KEY_MAX, false),
	lowerCaseLetters(ALPHA_CASE_NUM, false),
	upperCaseLetters(ALPHA_CASE_NUM, false),
...
void BoxEvents::fillSpecificKeys(SDL_Keycode keycode, bool isPressed)
{
	...
	{
		specificKeys[BOX_EV_ESCAPE] = isPressed;
	}
	else if( keycode == SDLK_BACKSPACE )
	{
		specificKeys[BOX_EV_BACKSPACE] = isPressed;
	}
	else if( keycode == SDLK_RETURN || keycode == SDLK_KP_ENTER )
	{
		specificKeys[BOX_EV_RETURN] = isPressed;
	}
	else if( keycode == SDLK_MINUS || keycode == SDLK_KP_MINUS )
	{
		specificKeys[BOX_EV_MINUS] = isPressed;
	}
	else if( keycode == SDLK_SPACE )
	{
		specificKeys[BOX_EV_SPACEBAR] = isPressed;
	}
	else if( keycode == SDLK_KP_PERIOD || keycode == SDLK_PERIOD )
	{
		specificKeys[BOX_EV_DOT] = isPressed;
	}
}
...
{
	std::fill(lowerCaseLetters.begin(), lowerCaseLetters.end(), false);
	std::fill(upperCaseLetters.begin(), upperCaseLetters.end(), false);
