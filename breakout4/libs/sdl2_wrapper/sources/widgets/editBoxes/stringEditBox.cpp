...
{
	if( events.getSpecificKey(BOX_EV_BACKSPACE) && false == ref.get().empty() )
	{
		ref.get().pop_back();
		hasStringChanged = true;
...
{
	if( events.getSpecificKey(BOX_EV_RETURN) && false == ref.get().empty() )
	{
		quitEdition = true;
...
{
	if( events.getSpecificKey(BOX_EV_SPACEBAR) )
	{
		ref.get() += ' ';
		hasStringChanged = true;
		events.setSpecificKeyToFalse(BOX_EV_SPACEBAR);
	}
}

void StringEditBox::appendDotToString()
{
	if( events.getSpecificKey(BOX_EV_DOT) )
	{
		ref.get() += '.';
		hasStringChanged = true;
		events.setSpecificKeyToFalse(BOX_EV_DOT);
	}
}
