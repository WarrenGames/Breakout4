...
{
	if( !isOnlyMinusCharInSintString(stringOfSint) && !stringOfSint.empty() 
	&& ( (!isUserMouseInBox(boxRect.getMainRect(), events.getMousePosition() ) && events.getMouseLeftButtonState() ) || events.getSpecificKey(BoxEventEscape) ) )
		quitEdition = true;
}
