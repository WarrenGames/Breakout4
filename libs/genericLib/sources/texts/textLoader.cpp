...
TextsBlocks::TextsBlocks(LogFile& log, const std::string& filePath, std::size_t expectedStringsNumber):
	errorString{ErrorString},
	isLoadingPerfect{true}
{
	if( std::ifstream languageFile{filePath} )
...
void TextsBlocks::initializeTexts(LogFile& log, const std::string& filePath, std::size_t expectedStringsNumber)
{
	errorString = ErrorString;
	isLoadingPerfect = true;
	if( std::ifstream languageFile{ filePath } )
...
void TextsBlocks::checkExpectedLoadedTexts(LogFile& log, const std::string& filePath, std::size_t expectedStringsNumber)
{
	if( texts.size() != expectedStringsNumber )
	{
		log << std::string{"Error: loading lang file '" + filePath + "' failed because size: " + std::to_string(texts.size() )
										+ " is different from expected " + std::to_string(expectedStringsNumber) + " texts"};
		isLoadingPerfect = false;
	}
}

void TextsBlocks::checkExpectedLoadedTexts(std::string& logString, const std::string& filePath, std::size_t expectedStringsNumber)
{
	if( texts.size() != expectedStringsNumber )
	{
