...
void rectCollision::startCollisionTest(spriteMove::ObjectMoving& objAMoving, const SpriteSize& objASize, const SDL_Rect& objBRect, int cornerInterval, const SoundPlayer& soundPlayer, 
										unsigned soundIndex, unsigned groupTag)
{
	if( rectCollision::doesAObjectCollideWithRect(objAMoving, objASize, objBRect) )
	{
		...
		soundPlayer.playSoundOnGroup(soundIndex, groupTag);
	}
}
