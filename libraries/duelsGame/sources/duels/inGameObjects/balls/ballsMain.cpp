...
void duels::BallsThings::update(const MatrixTemp2D<BrickData>& levelGrid, duels::LevelOptions& levelOptions, CoinsThings& coins, IntRandomGenerator& randomGen, 
								const SoundPlayer& soundPlayer, std::vector< duels::ScoreDraw >& scoreDrawing, unsigned& givenPoints)
{
	moveBalls();
	accelarateBallsSpeed();
	checkCollisionWithGrid(levelGrid, soundPlayer);
	setCanCreateBall();
...
void duels::BallsThings::checkCollisionWithGrid(const MatrixTemp2D<BrickData>& levelGrid, const SoundPlayer& soundPlayer)
{
	if( collisionBools.canCheckCollisionWithGrid )
	{
		for( auto &ball : ballsData )
		{
			if( ball.collision.isEffective)
			{
				...
				playSoundIfCollision(soundPlayer, wasThereCol);
			}
		}
...
{
	return static_cast<unsigned>( std::count_if(ballsData.cbegin(), ballsData.cend(), [](auto const &ball){return ball.collision.isEffective;} ) );
}
...
{
	return std::any_of(ballsData.cbegin(), ballsData.cend(), [spawnPoint](auto const& ball){return isMoveObjectOverSpawnPoint(ball.move, SpriteSize{SQR_SIZE, SQR_SIZE}, spawnPoint);} );
}
...
void duels::BallsThings::playSoundIfCollision(const SoundPlayer& soundPlayer, bool wasThereCol)
{
	if( wasThereCol )
	{
		soundPlayer.playSoundOnGroup(duels::SoundCollisionBallWithBackground, duels::GroupTagBallCollision);
	}
}