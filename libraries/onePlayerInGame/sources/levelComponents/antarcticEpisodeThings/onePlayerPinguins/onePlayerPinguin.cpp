											...
											demos::DemoAntarcticPackage& antarcticPackage)
{
	for( auto &ping : pinguins )
	{
		ping.makePinguinQuack(racketRect, collisionRect, playerLeftState, playerRightState, sounds, antarcticPackage);
	}
}
...
void OnePlayerPinguin::playRecordedQuackSounds(demos::DemoAntarcticPackage& antarcticPackage, const SoundHandler& sounds)
{
	if( antarcticPackage.demoKind == demos::GameIsDemo )
	{
		...
		{
			sounds.playSound( static_cast<std::size_t>( antarcticPackage.pinguinsQuacks.getLastElement().soundEnum ) );
