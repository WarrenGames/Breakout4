...
void LevelComponents::commonUpdate(Essentials& essentials, LevelInputs& levelInputs, PlayerData& playerData, bool& quitGame)
{
	...
	detectLiveLossWithScreenBottom(ball, levelEndType, playerData, sounds, infoBar, playedTimeCounter);
...
{
	commonUpdate(essentials, levelInputs, playerData, quitGame);
	actWithPlayerInputs(levelInputs, antarcticPackage.pinguinsData.atLeastOnePinguinOnRacketRight(racket.rect), antarcticPackage.pinguinsData.atLeastOnePinguinOnRacketLeft(racket.rect), playerData);
...
void LevelComponents::playSoundsWithDemoStack(const PlayerData& playerData)
{
	if( playerData.demoKind == demos::GameIsDemo )
	{
		...
		{
			sounds.playSound( static_cast<std::size_t>( soundsStack.getLastElement().soundEnum ) );
...
void LevelComponents::updatePinguins(AntarcticPackage& antarcticPackage, LevelInputs& levelInputs)
{
	...
	antarcticPackage.pinguinsData.makePinguinsQuack(racket.rect, levelInputs.getFuncState(onePlGame::InputMoveLeft), levelInputs.getFuncState(onePlGame::InputMoveRight), sounds, 
														antarcticPackage.antarcticDemoPackage);
	...
	antarcticPackage.pinguinsData.playRecordedQuackSounds(antarcticPackage.antarcticDemoPackage, sounds);
}
