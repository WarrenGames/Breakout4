...
void InfoBar::createGraphicalBarTexts(Essentials& essentials)
{
	if( fancyFont )
	{
		if( const TextsBlocks infosBarLangTexts{essentials.logs.error, path::getLanguageFile(essentials.language, file::OnePlayerInfoBar), infoBarTexts::Max} )
		{
			std::vector< Offset > textsPositions{ Offset{ SQR_SIZE, SQR_SIZE/4}, 
																	Offset{ GameScreenWidth/2 + SQR_SIZE * 2, SQR_SIZE/4},
																	Offset{ SQR_SIZE, SQR_SIZE * 3 / 4}, 
																	Offset{ GameScreenWidth/2 + SQR_SIZE * 2, SQR_SIZE * 3 /4} };
...
{
	blackSquares[index].texture.loadTextureSquare(essentials.logs, essentials.rndWnd, BlackColor, SQR_SIZE * 3, SQR_SIZE* 3 / 8 );
