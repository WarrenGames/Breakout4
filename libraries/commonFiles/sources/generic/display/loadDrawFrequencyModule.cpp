...
{
	if( std::ifstream drawFreqFile{ path::getConfigDirFile(prefPath, file::DrawFrequencyFile) } )
	{
		if( drawFreqFile >> framesPerSecond )
		{
			...
			logs.error.wrReadErrorMessage(path::getConfigDirFile(prefPath, file::DrawFrequencyFile), "frames per second" );
		}
	}
	else{
		isLoadingPerfect = false;
		logs.error.wrFileOpeningError(path::getConfigDirFile(prefPath, file::DrawFrequencyFile), "read frames per second rate.");
	}
}
