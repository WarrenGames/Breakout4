...
struct ScoreSample
{
	...
	~ScoreSample() = default;
	ScoreSample( const ScoreSample& ) = default;
	ScoreSample& operator= ( const ScoreSample& ) = default;
...
private:
	void saveScoresToFile(Essentials& essentials, const fs::path& scoreFilePath, unsigned skill) const;
	void loadScores(Essentials& essentials, unsigned campaignType);
	void loadScoresList(Essentials& essentials, unsigned skillNumber, const fs::path& scoreFilePath);
