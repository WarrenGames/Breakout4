...
{
	if( std::ofstream musicConfiguration{ path::getConfigDirFile(essentials.prefPath, file::MusicConfigFile) } )
	{
		musicConfiguration << isMusicActive << " " << volBar.getVolume();
	}
	else{
		essentials.logs.error.wrFileOpeningError( path::getConfigDirFile(essentials.prefPath, file::MusicConfigFile), "save music configuration for the options.");
	}
}
...
void MusicConfig::readDataFile(Essentials& essentials)
{
	if( std::ifstream soundFile{ path::getConfigDirFile(essentials.prefPath, file::MusicConfigFile) } )
	{
		...
		if( soundFile >> isMusicActive >> musicVolume )
		{
			...
			essentials.logs.error.wrReadErrorMessage( path::getConfigDirFile(essentials.prefPath, file::MusicConfigFile), "'is music active' boolean and music volume.");
			isLoadingPerfect = false;
		}
	}
	else{
		essentials.logs.error.wrFileOpeningError( path::getConfigDirFile(essentials.prefPath, file::MusicConfigFile), "read music configuration.");
		isLoadingPerfect = false;
	}
}
