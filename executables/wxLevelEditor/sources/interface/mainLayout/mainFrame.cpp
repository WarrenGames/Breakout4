	...
	lastVBoxSizer{ new wxBoxSizer{ wxVERTICAL } }
{
	...
	Bind(wxEVT_COMMAND_MENU_SELECTED, [this](wxCommandEvent& event){ saveCurrentOnePlayerGrid(event); }, fileHandling::SaveFileAs );
	Bind(wxEVT_COMMAND_MENU_SELECTED, [this](wxCommandEvent& event){ saveToSameFile(event); }, fileHandling::SaveToSameFile );
...
void MainFrame::loadGridFromFile(wxCommandEvent& event)
{
	...
	catch( const std::exception& e )
	{
		if( logWindow )
		{
			*logWindow << "Error: exception thrown with this argument: " << e.what() << '\n';
		}
	}
}
...
void MainFrame::saveToSameFile(wxCommandEvent& event)
{
	try{
		if( onePGrid )
		{
			onePGrid->saveGridToSameFile(*logWindow );
			*logWindow << "Saved grid file with success.\n"; 
		}
	}
	catch( const std::exception& e )
	{
